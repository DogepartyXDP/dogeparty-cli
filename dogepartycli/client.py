...
APP_NAME = 'dogeparty-client'

CONFIG_ARGS = [
    [('-v', '--verbose'), {'dest': 'verbose', 'action': 'store_true', 'help': 'sets log level to DEBUG instead of WARNING'}],
    [('--testnet',), {'action': 'store_true', 'default': False, 'help': 'use {} testnet addresses and block numbers'.format(config.DOGE_NAME)}],
    [('--testcoin',), {'action': 'store_true', 'default': False, 'help': 'use the test {} network on every blockchain'.format(config.XDP_NAME)}],
    [('--regtest',), {'action': 'store_true', 'default': False, 'help': 'use {} regtest addresses and block numbers'.format(config.DOGE_NAME)}],
    ...
    [('--dogeparty-rpc-connect',), {'default': 'localhost', 'help': 'the hostname or IP of the Dogeparty JSON-RPC server'}],
    [('--dogeparty-rpc-port',), {'type': int, 'help': 'the port of the Dogeparty JSON-RPC server'}],
    [('--dogeparty-rpc-user',), {'default': 'rpc', 'help': 'the username for the Dogeparty JSON-RPC server'}],
    [('--dogeparty-rpc-password',), {'help': 'the password for the Dogeparty JSON-RPC server'}],
    [('--dogeparty-rpc-ssl',), {'default': False, 'action': 'store_true', 'help': 'use SSL to connect to the Dogeparty server (default: false)'}],
    [('--dogeparty-rpc-ssl-verify',), {'default': False, 'action': 'store_true', 'help': 'verify SSL certificate of the Dogeparty server; disallow use of self‐signed certificates (default: false)'}],

    [('--wallet-name',), {'default': 'dogecoincore', 'help': 'the wallet name to connect to'}],
    ...
    [('--wallet-user',), {'default': 'dogecoinrpc', 'help': 'the username used to communicate with wallet'}],
    ...
    [('--fee-per-kb',), {'type': D, 'default': D(config.DEFAULT_FEE_PER_KB / config.UNIT), 'help': 'fee per kilobyte, in {}'.format(config.DOGE)}],
    [('--regular-dust-size',), {'type': D, 'default': D(config.DEFAULT_REGULAR_DUST_SIZE / config.UNIT), 'help': 'value for dust Pay‐to‐Pubkey‐Hash outputs, in {}'.format(config.DOGE)}],
    [('--multisig-dust-size',), {'type': D, 'default': D(config.DEFAULT_MULTISIG_DUST_SIZE / config.UNIT), 'help': 'for dust OP_CHECKMULTISIG outputs, in {}'.format(config.DOGE)}],
    [('--op-return-value',), {'type': D, 'default': D(config.DEFAULT_OP_RETURN_VALUE / config.UNIT), 'help': 'value for OP_RETURN outputs, in {}'.format(config.DOGE)}],
...
def main():
    ...
    parser = argparse.ArgumentParser(prog=APP_NAME, description='Dogeparty CLI for dogeparty-server', add_help=False)
    parser.add_argument('-h', '--help', dest='help', action='store_true', help='show this help message and exit')
    parser.add_argument('-V', '--version', action='version', version="{} v{}; {} v{}".format(APP_NAME, APP_VERSION, 'dogeparty-lib', config.VERSION_STRING))
    ...
    parser_send.add_argument('--no-use-enhanced-send', action='store_false', dest="use_enhanced_send", default=True, help='If set to false, compose a non-enhanced send with a dogecoin dust output')
    parser_send.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_sweep.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_dispenser.add_argument('--mainchainrate', required=True, help='the quantity of %s (decimal) this dispenser must receive to send the GIVEN_QUANTITY of the ASSET' % config.DOGE)
    parser_dispenser.add_argument('--give-quantity', required=True, help='the quantity of ASSET that you are giving for each MAINCHAINRATE of %s received' % config.DOGE)
    ...
    parser_dispenser.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_order.add_argument('--fee-fraction-required', default=config.DEFAULT_FEE_FRACTION_REQUIRED, help='the miners’ fee required for an order to match this one, as a fraction of the {} to be bought'.format(config.DOGE))
    parser_order_fees = parser_order.add_mutually_exclusive_group()
    parser_order_fees.add_argument('--fee-fraction-provided', default=config.DEFAULT_FEE_FRACTION_PROVIDED, help='the miners’ fee provided, as a fraction of the {} to be sold'.format(config.DOGE))
    parser_order_fees.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))

    parser_dogepay = subparsers.add_parser('{}pay'.format(config.DOGE).lower(), help='create and broadcast a *{}pay* message, to settle an Order Match for which you owe {}'.format(config.DOGE, config.DOGE))
    parser_dogepay.add_argument('--source', required=True, help='the source address')
    parser_dogepay.add_argument('--order-match-id', required=True, help='the concatenation of the hashes of the two transactions which compose the order match')
    parser_dogepay.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_issuance.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_broadcast.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_bet.add_argument('--wager', required=True, help='the quantity of XDP to wager')
    parser_bet.add_argument('--counterwager', required=True, help='the minimum quantity of XDP to be wagered by the user to bet against you, if he were to accept the whole thing')
    ...
    parser_bet.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_dividend.add_argument('--quantity-per-unit', required=True, help='the quantity of XDP to be paid per whole unit held of ASSET')
    ...
    parser_dividend.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))

    parser_burn = subparsers.add_parser('burn', help='destroy {} to earn XDP, during an initial period of time')
    parser_burn.add_argument('--source', required=True, help='the source address')
    parser_burn.add_argument('--quantity', required=True, help='quantity of {} to be burned'.format(config.DOGE))
    parser_burn.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_cancel.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_publish.add_argument('--startgas', required=True, type=int, help='the maximum quantity of {} to be used to pay for the execution (satoshis)'.format(config.XDP))
    parser_publish.add_argument('--endowment', required=True, type=int, help='quantity of {} to be transfered to the contract (satoshis)'.format(config.XDP))
    parser_publish.add_argument('--code-hex', required=True, type=str, help='the hex‐encoded contract (returned by `serpent compile`)')
    parser_publish.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))
    ...
    parser_execute.add_argument('--startgas', required=True, type=int, help='the maximum quantity of {} to be used to pay for the execution (satoshis)'.format(config.XDP))
    parser_execute.add_argument('--value', required=True, type=int, help='quantity of {} to be transfered to the contract (satoshis)'.format(config.XDP))
    ...
    parser_execute.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))

    parser_destroy = subparsers.add_parser('destroy', help='destroy a quantity of a Dogeparty asset')
    ...
    parser_destroy.add_argument('--fee', help='the exact {} fee to be paid to miners'.format(config.DOGE))

    parser_address = subparsers.add_parser('balances', help='display the balances of a {} address'.format(config.XDP_NAME))
    parser_address.add_argument('address', help='the address you are interested in')

    parser_asset = subparsers.add_parser('asset', help='display the basic properties of a {} asset'.format(config.XDP_NAME))
    parser_asset.add_argument('asset', help='the asset you are interested in')

    parser_wallet = subparsers.add_parser('wallet', help='list the addresses in your backend wallet along with their balances in all {} assets'.format(config.XDP_NAME))

    parser_pending = subparsers.add_parser('pending', help='list pending order matches awaiting {}payment from you'.format(config.DOGE))

    parser_getrows = subparsers.add_parser('getrows', help='get rows from a Dogeparty table')
    ...
    clientapi.initialize(testnet=args.testnet, testcoin=args.testcoin, regtest=args.regtest, customnet=args.customnet,
                        dogeparty_rpc_connect=args.dogeparty_rpc_connect, dogeparty_rpc_port=args.dogeparty_rpc_port,
                        dogeparty_rpc_user=args.dogeparty_rpc_user, dogeparty_rpc_password=args.dogeparty_rpc_password,
                        dogeparty_rpc_ssl=args.dogeparty_rpc_ssl, dogeparty_rpc_ssl_verify=args.dogeparty_rpc_ssl_verify,
