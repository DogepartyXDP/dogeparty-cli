...
from dogepartylib.lib import config, check
...
def api(method, params=None):
    return rpc(config.DOGEPARTY_RPC, method, params=params, ssl_verify=config.DOGEPARTY_RPC_SSL_VERIFY)
...
def is_divisible(asset):
    if asset in (config.DOGE, config.XDP, 'leverage', 'value', 'fraction', 'price', 'odds'):
        return True
    else:
...
def bootstrap(testnet=False, overwrite=True, ask_confirmation=False, quiet=False):
    ...
    if testnet:
        if check.CONSENSUS_HASH_VERSION_TESTNET < 7:
            BOOTSTRAP_URL = 'https://dogeparty.net/bootstrap/dogeparty-db-testnet.latest.tar.gz'
        else:
            BOOTSTRAP_URL = 'https://dogeparty.net/bootstrap/dogeparty-db-testnet-{}.latest.tar.gz'.format(check.CONSENSUS_HASH_VERSION_TESTNET)
        TARBALL_PATH = os.path.join(tempfile.gettempdir(), 'dogepartyd-testnet-db.latest.tar.gz')
        DATABASE_PATH = os.path.join(data_dir, '{}.testnet.db'.format(config.APP_NAME))
    else:
        if check.CONSENSUS_HASH_VERSION_MAINNET < 3:
            BOOTSTRAP_URL = 'https://dogeparty.net/bootstrap/dogeparty-db.latest.tar.gz'
        else:
            BOOTSTRAP_URL = 'https://dogeparty.net/bootstrap/dogeparty-db-{}.latest.tar.gz'.format(check.CONSENSUS_HASH_VERSION_MAINNET)
        TARBALL_PATH = os.path.join(tempfile.gettempdir(), 'dogepartyd-db.latest.tar.gz')
