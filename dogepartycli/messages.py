...
def prepare_args(args, action):
    ...
    # common
    if args.fee:
        args.fee = util.value_in(args.fee, config.DOGE)
    ...
    if action == 'dispenser':
        ...
        args.mainchainrate = util.value_in(args.mainchainrate, config.DOGE)
    ...
    if action == 'order':
        ...
        if args.give_asset == config.DOGE:
            args.fee_required = 0
            ...
            print('Fee provided: {} {}'.format(util.value_out(args.fee_provided, config.DOGE), config.DOGE))
        elif args.get_asset == config.DOGE:
            args.fee_provided = 0
            ...
            print('Fee required: {} {}'.format(util.value_out(args.fee_required, config.DOGE), config.DOGE))
        else:
            args.fee_required = 0
            args.fee_provided = 0
    ...
    if action == 'bet':
        ...
        args.wager = util.value_in(args.wager, config.XDP)
        args.counterwager = util.value_in(args.counterwager, config.XDP)
    ...
    # dividend
    if action == 'dividend':
        args.quantity_per_unit = util.value_in(args.quantity_per_unit, config.XDP)

    # burn
    if action == 'burn':
        args.quantity = util.value_in(args.quantity, config.DOGE)

    # execute
    if action == 'execute':
        args.value = util.value_in(args.value, 'XDP')
        args.startgas = util.value_in(args.startgas, 'XDP')
    ...
    if action == 'rps':
        ...
        args.wager = util.value_in(args.wager, 'XDP')
