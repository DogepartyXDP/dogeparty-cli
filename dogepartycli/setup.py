...
def extract_old_config():
    ...
    old_appdir = appdirs.user_config_dir(appauthor='Dogeparty', appname='dogepartyd', roaming=True)
    old_configfile = os.path.join(old_appdir, 'dogepartyd.conf')
...
def server_to_client_config(server_config):
    ...
    config_keys = {
        ...
        'rpc-host': 'dogeparty-rpc-connect',
        'rpc-port': 'dogeparty-rpc-port',
        'rpc-user': 'dogeparty-rpc-user',
        'rpc-password': 'dogeparty-rpc-password'
    }
...
def generate_config_files():
    ...
    configdir = appdirs.user_config_dir(appauthor=config.XDP_NAME, appname=config.APP_NAME, roaming=True)
...
def before_py2exe_build(win_dist_dir):
    ...
    for exe_name in ['client', 'server']:
        shutil.copy('dogepartycli/__init__.py', 'dogeparty-{}.py'.format(exe_name))
        with open('dogeparty-{}.py'.format(exe_name), 'a') as fp:
...
def after_py2exe_build(win_dist_dir):
    ...
    for exe_name in ['client', 'server']:
        os.remove('dogeparty-{}.py'.format(exe_name))
...
def bootstrap(overwrite=True, ask_confirmation=False):
    if ask_confirmation:
        question = 'Would you like to bootstrap your local Dogeparty database from `https://dogeparty.net/bootstrap/`? (y/N): '
