...
def main():
    ...
    parser = argparse.ArgumentParser(prog=APP_NAME, description='Server for the {} protocol'.format(config.XDP_NAME), add_help=False)
    ...
    parser_kickstart = subparsers.add_parser('kickstart', help='rapidly build database by reading from Dogecoin Core blockchain')
    parser_kickstart.add_argument('--dogecoind-dir', help='Dogecoin Core data directory')
    ...
    log.set_up(log.ROOT_LOGGER, verbose=args.verbose, console_logfilter=os.environ.get('DOGEPARTY_LOGGING', None))
    ...
    if args.action in COMMANDS_WITH_DB or args.action in COMMANDS_WITH_CONFIG:
        init_args = dict(database_file=args.database_file,
                                ...
                                force=args.force, verbose=args.verbose, console_logfilter=os.environ.get('DOGEPARTY_LOGGING', None),
    ...
    if args.action == 'reparse':
        ...
        server.kickstart(db, dogecoind_dir=args.dogecoind_dir)
