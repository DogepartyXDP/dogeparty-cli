...
from dogepartycli.wallet import dogecoincore, dogewallet
...
def get_doge_balances():
    for address, doge_balance in WALLET().get_doge_balances():
    	yield [address, doge_balance]
...
def get_doge_balance(address):
    return WALLET().get_doge_balance(address)
...
def wallet():
    ...
    for bunch in get_doge_balances():
        address, doge_balance = bunch
        add_total(address, 'DOGE', doge_balance)
...
def pending():
    ...
    awaiting_doges = api('get_order_matches', {'filters': filters, 'filterop': 'OR', 'status': 'pending'})
    return awaiting_doges
